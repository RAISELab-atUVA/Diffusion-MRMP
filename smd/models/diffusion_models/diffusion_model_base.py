...
from smd.models.diffusion_models.helpers import cosine_beta_schedule, Losses, exponential_beta_schedule
from smd.models.diffusion_models.sample_functions import extract, apply_hard_conditioning, apply_cross_conditioning, guide_gradient_steps, \
    ddpm_sample_fn
...
class GaussianDiffusionModel(nn.Module, ABC):
    ...
    def p_sample_loop(self, shape,
        ...
        x_projected = None
        for i in reversed(range(0, n_diffusion_steps)):
            t = make_timesteps(batch_size, i, device)
            x, values = sample_fn(self, x, hard_conds, context, t, **sample_kwargs)
            x = apply_hard_conditioning(x, hard_conds)
            is_first_step = 1 if i == n_diffusion_steps - 1 else 0


            projection_step = proj_params['projection_step']
            if i == 0 or i in projection_step:
                print(CYAN, "Projection at step", i, RESET)
                x, succcess_projection = apply_projection_alm(x, projection_info, hard_conds,is_first_step,init_traj4proj,proj_params)
                if succcess_projection > 0.5:
                    x_projected = x
                if i == 0 and succcess_projection < 0.5:
                    print(RED, "Projection failed at Final step", RESET)
                    try_times = 0
                    while succcess_projection < 0.5:
                        x, values = sample_fn(self, x, hard_conds, context, t, **sample_kwargs)
                        x = apply_hard_conditioning(x, hard_conds)
                        x, succcess_projection = apply_projection_alm(x, projection_info, hard_conds,is_first_step,init_traj4proj,proj_params)
                        try_times += 1
                        if try_times > 0:
                            if x_projected is not None:
                                ...
                                print(RED, "Projection failed after 2 tries", RESET)
                                break
